...
function getSumBetweenNumbers(n1, n2) {
  return n1 + 1 > n2 ? n1 : n1 + getSumBetweenNumbers(n1 + 1, n2);
}
...
function doRectanglesOverlap(r1, r2) {
  return r1.width + r1.left > r2.left && r1.height + r1.top > r2.top;
}
...
function isInsideCircle(circle, point) {
  ...
  return (x1 ** 2 + y1 ** 2) ** 0.5 < circle.radius;
}
...
function findFirstSingleChar(str) {
  return str
    .split('')
    .filter((el, ind, arr) => arr.indexOf(el) === arr.lastIndexOf(el))[0];
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startSymb}${x1}, ${x2}${endSymb}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  ccnArr = ccnArr.map((val, ind) => {
    if ((ccnArr.length - ind) % 2 === 0) {
      const x = val * 2;
      return x > 9 ? x - 9 : +x;
    }
    return +val;
  });
  return ccnArr.reduce((prev, cur) => prev + cur, 0) % 10 === 0;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  pathesArr[0].map((el, ind) => {
    const tempArr = pathesArr.filter((element) => el === element[ind]);
